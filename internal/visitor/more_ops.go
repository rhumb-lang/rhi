...
func (b *ASTBuilder) VisitApplicative(ctx *grammar.ApplicativeContext) interface{} {
	...
	var op ast.OpType
	switch opCtx.(type) {
	case *grammar.FunctionContext:
		op = ast.OpMakeFn
	case *grammar.MethodContext:
		op = ast.OpBindFn
	case *grammar.LetFunctionContext:
		op = ast.OpLetFn
	default:
		op = ast.OpMakeFn
	}
...
func (b *ASTBuilder) VisitConditional(ctx *grammar.ConditionalContext) interface{} {
	...
	var op ast.OpType
	switch opCtx.(type) {
	case *grammar.PipeContext:
		op = ast.OpPipe
	case *grammar.DefaultContext:
		op = ast.OpCoalesce
	case *grammar.ForeachContext:
		op = ast.OpForeach
	case *grammar.WhileContext:
		op = ast.OpWhile
	case *grammar.ThenContext:
		op = ast.OpIfTrue
	case *grammar.ElseContext:
		op = ast.OpIfFalse
	default:
		op = ast.OpIfTrue
	}
...
func (b *ASTBuilder) VisitDestructive(ctx *grammar.DestructiveContext) interface{} {
	left := toExpr(b.Visit(ctx.Expression(0)))
	right := toExpr(b.Visit(ctx.Expression(1)))

	return &ast.BinaryExpression{Left: left, Op: ast.OpDestruct, Right: right}
}
...
func (b *ASTBuilder) VisitPower(ctx *grammar.PowerContext) interface{} {
	...
	var op ast.OpType
	switch opCtx.(type) {
	case *grammar.ExponentContext:
		op = ast.OpPow
	case *grammar.RootExtractionContext:
		op = ast.OpRoot
	case *grammar.RangeContext:
		op = ast.OpRange
	case *grammar.ScientificContext:
		op = ast.OpSciNot
	default:
		op = ast.OpPow
	}
...
func (b *ASTBuilder) VisitPrefix(ctx *grammar.PrefixContext) interface{} {
	...
	var op ast.OpType
	switch opCtx.(type) {
	case *grammar.EmptyPrefixContext:
		op = ast.OpIsEmpty
	case *grammar.FreezePrefixContext:
		op = ast.OpFreeze
	case *grammar.CopyPrefixContext:
		op = ast.OpCopy
	case *grammar.ToNumberPrefixContext:
		op = ast.OpToNum
	case *grammar.NegateNumberPrefixContext:
		op = ast.OpNegNum
	case *grammar.ToTruthPrefixContext:
		op = ast.OpToBool
	case *grammar.NegateTruthPrefixContext:
		op = ast.OpNegBool
	case *grammar.VariadicPrefixContext:
		op = ast.OpSpread
	case *grammar.ArgumentPrefixContext:
		op = ast.OpGetParams
	case *grammar.SignalOutwardPrefixContext:
		op = ast.OpSignal
	case *grammar.SignalInwardPrefixContext:
		op = ast.OpReply
	default:
		op = ast.OpEq
	}
